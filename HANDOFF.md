...
- 当前 checkout 复核已完成：backend 全量回归 `518 passed, 9 skipped, 17 warnings, 11 subtests passed in 414.08s`，frontend typecheck/build 均 exit `0`。pytest、npm cache、日志与浏览器/Agent 临时数据继续固定在 `H:\tmp\offeru`；邮箱 Registry 审计确认 10 个只读操作、同步需要 `mailbox:read` 与用户确认，transport 契约另有 `9 passed`，未发现 send/reply/delete/archive/mark-read 操作。证据 `H:\tmp\offeru\full-regression-resume-index-fix-20260910\pytest.log`、`H:\tmp\offeru\frontend-validation-current-20260911`、`H:\tmp\offeru\audit-email-boundary-20260911.json`、`H:\tmp\offeru\email-read-only-transport-20260909.log`。
- 2026-09-10 Resume UI/机械解析阻塞已修复并真实复核：`ProfileOnboarding` 的原版机械解析按钮现在使用隐藏 PDF/DOCX 文件输入，后端识别 Education/Experience/Projects/Skills 等英文章节标题，390×844 managed Chromium 无头流程完成上传 H 盘 fixture、审核 4 条候选并生成档案，写入 8 个 Profile section；横向溢出、console/page error、坏响应均为 `0`。定向测试 `4 passed`，完整 backend 回归 `519 passed, 9 skipped, 17 warnings, 11 subtests passed in 429.24s`。证据 `H:\tmp\offeru\resume-browser-ui-20260912e\resume-import-browser-result.json`、`H:\tmp\offeru\profile-resume-english-headings-20260912\pytest.log`、`H:\tmp\offeru\full-regression-resume-english-20260912\pytest.log`；仍为 synthetic fixture，不是用户 Resume。
- 2026-09-10 邮箱回补主题查询补齐投递/招聘/录用/拒信/申请已提交/跟进及英文短语，仍保持 Gmail GET-only 与正文相关性过滤；邮箱切片 `10 passed`，完整 backend 回归 `520 passed, 9 skipped, 17 warnings, 11 subtests passed in 424.43s`，架构审计 `0 findings`。证据 `H:\tmp\offeru\email-query-boundary-20260910\pytest.log`、`H:\tmp\offeru\full-regression-email-query-20260910\pytest.log`、`H:\tmp\offeru\architecture-audit-email-query-20260910\audit.log`；真实 Gmail OAuth 仍未完成。
- 2026-09-10 同步状态投影对历史 `EmailAccount.last_error`/`EmailSyncRun.error` 统一做邮箱、电话和凭据脱敏；邮箱切片 `11 passed`，完整 backend 回归 `521 passed, 9 skipped, 17 warnings, 11 subtests passed in 429.93s`。证据 `H:\tmp\offeru\email-status-redaction-20260910\pytest.log`、`H:\tmp\offeru\full-regression-email-status-redaction-20260910\pytest.log`；真实 Gmail OAuth 仍未完成。
- 2026-09-10 Codex Bridge conformance 已升级为完整 Career context 读取门：真实 H 盘隔离任务自主调用 Profile、Resume、Job、learning observations、pending memory、Profile evolution、email accounts、email sync runs 共 10 个只读 Operation；`missing_context_reads=[]`、`mutations=0`、`grounding_verified=true`。相关切片 `65 passed`，完整 backend 回归 `521 passed, 9 skipped, 17 warnings, 11 subtests passed in 425.81s`，frontend typecheck/build 退出码均为 `0`，架构审计 `0 findings`。证据 `H:\tmp\offeru\codex-bridge-context-read-real-20260910.json`、`H:\tmp\offeru\bridge-context-read-targeted-20260910.log`、`H:\tmp\offeru\full-regression-bridge-context-20260910\pytest.log`；该 Career context 使用合成隔离数据，真实 Resume/Gmail OAuth 仍未完成。
- 2026-09-10 Codex 故障收敛门补证：H 盘隔离 CareerTask auth blocked→retry、provider timeout failed→retry、backend restart recovery→retry 全部通过且事件无重复；真实 Codex app-server 只终止本次创建的一个 PID，turn 在 `2.432s` 内 fail-closed，未假报 completed。`CodexMainLoopAdapter` 现会把原生 app-server EOF/process failure 传递给 pending request/turn waiter，定向测试 `1 passed`，完整 backend 回归 `522 passed, 9 skipped, 17 warnings, 11 subtests passed in 423.21s`，架构审计 `0 findings`。证据 `H:\tmp\offeru\failure-recovery-real-20260910\result.log`、`H:\tmp\offeru\codex-kill-real-20260910\result.json`、`H:\tmp\offeru\codex-adapter-process-failure-20260910\pytest.log`、`H:\tmp\offeru\full-regression-codex-eof-20260910\pytest.log`，提交 `539a985`。所有 CWD、日志、数据库和临时文件继续位于 `H:\tmp\offeru`；真实 Resume/Gmail OAuth 与真实来源长期演化仍是外部门。
- 2026-09-10 Hosted Codex 认证门补强：隔离 `CODEX_HOME` 的真实 `account/read` 返回未登录时，`execute_deep_task` 在 `4.999s` 内持久化为 `failed` 并返回 `Codex authentication required`；HostedExecutor 关闭顺序先等待 EOF，认证失败/进程终止不再留下 Proactor `ResourceWarning`。定向 runtime 回归 `19 passed, 2 subtests passed`，完整 backend 回归 `523 passed, 9 skipped, 17 warnings, 11 subtests passed in 426.89s`，架构审计 `0 findings`；真实 HostedExecutor PID 终止复跑在 `1.984s` 内为 durable `failed`，无 `session.completed`。证据 `H:\tmp\offeru\codex-auth-unavailable-real-20260910c\probe.log`、`H:\tmp\offeru\codex-hosted-kill-real-20260910b\probe.log`、`H:\tmp\offeru\codex-auth-gate-tests-20260910b\pytest.log`、`H:\tmp\offeru\full-regression-codex-auth-gate-20260910\pytest.log`，提交 `dd41378`。真实 Resume/Gmail OAuth 与真实来源长期演化仍未完成。
- 2026-09-10 Direct Codex Bridge 同步复核：`CodexMainLoopAdapter` 现在也在每次 Bridge turn 前检查 provider-owned `account/read`；登录态真实 conformance 仍自主发现并读取 10 个只读 Operation，`missing_context_reads=[]`、`mutations=0`、`grounding_verified=true`。定向 Bridge/runtime 回归 `49 passed, 2 subtests passed`，最新完整 backend 回归 `524 passed, 9 skipped, 17 warnings, 11 subtests passed in 428.32s`，架构审计 `0 findings`。证据 `H:\tmp\offeru\codex-bridge-context-read-auth-gate-20260910\conformance.log`、`H:\tmp\offeru\codex-direct-auth-tests-20260910\pytest.log`、`H:\tmp\offeru\full-regression-codex-bridge-auth-20260910\pytest.log`，提交 `eea75df`。真实 Resume/Gmail OAuth 与真实来源长期演化仍未完成。
- 2026-09-10 Direct Bridge 未认证 fail-closed 复核：隔离 `CODEX_HOME` 下 Bridge 在任何 Registry 调用前返回 `ok=false`、`failure_reason=Codex authentication required`、`tool_calls=0`、`mutations=0`；未产生 completed 结果。证据 `H:\tmp\offeru\codex-bridge-auth-unavailable-20260910\result.json`。
- 2026-09-10 Codex app-server 握手顺序复核：Hosted 与 Direct Bridge 均遵循 `initialize → initialized → account/read → turn`；登录态真实 nonce/JSON/stream probe 与完整 Bridge Career context conformance 均通过。当前 HEAD 全量 backend `524 passed, 9 skipped, 17 warnings, 11 subtests passed in 430.51s`，定向 Bridge/runtime `49 passed, 2 subtests passed`，架构审计 `0 findings`。证据 `H:\tmp\offeru\codex-live-handshake-order-20260910\conformance.log`、`H:\tmp\offeru\codex-bridge-context-read-auth-gate-20260910\conformance.log`、`H:\tmp\offeru\full-regression-codex-handshake-order-20260910\pytest.log`，提交 `44aca58`。
- 2026-09-10 Agent 能力矩阵的认证状态分类已收紧：显式 `authentication required`/`login required` 失败现在投影为 `BLOCKED_AUTH`，不再混入普通 `ERROR`；H 盘隔离真实 Codex 未登录 live probe 返回 `BLOCKED_AUTH`，定向分类回归 `6 passed`，当前带默认 Profile 的 backend 全量回归 `525 passed, 9 skipped, 17 warnings, 11 subtests passed in 433.26s`。证据 `H:\tmp\offeru\agent-conformance-auth-required-20260910\conformance.log`、`H:\tmp\offeru\agent-conformance-auth-classification-20260910\pytest.log`、`H:\tmp\offeru\full-regression-auth-classification-seeded-20260910\pytest.log`，提交 `f381079`。真实 Resume/Gmail OAuth 与真实来源长期演化仍未完成。
- Resume 导入首条候选的证据关联已修复：`candidate_index=0` 不再被错误当作缺失，定向 Resume review 回归 `11 passed`。H 盘 fixture 端到端执行显示 8/8 候选均保持 pending、8/8 均有 `observation_id` 与 `memory_proposal_id`，正式 `ProfileSection` 写入为 `0`，确认前不会污染档案；证据 `H:\tmp\offeru\profile-resume-index-fix-20260910\pytest.log`、`H:\tmp\offeru\resume-parser-execution-20260910d\result.json`。该执行是 synthetic fixture，不是用户 Resume。
- 外部输入预检 `H:\tmp\offeru\real-input-preflight-20260909.json` 只扫描 H 盘仓库/临时目录，3 个发现的简历均标为 fixture，真实 Resume 候选为 0；Gmail client/OAuth 仍未完成，C 盘没有用于测试的目录。
- 已修复拒绝记忆的长期幂等边界：同一来源重复不会重新进入 pending，新的独立来源才允许重新提出。定向 Career Memory/Profile 回归 `35 passed`，完整回归 `515 passed, 9 skipped, 17 warnings, 11 subtests`；synthetic stress 通过 3 个冲突、已确认独立证据加强（`strengthened_facts=1`）、3 次重放去重、拒绝/新证据重开和潜力假设隔离；演化报告类别计数为新增 7、加强 3、冲突 3、替代 3、拒绝 1、潜力假设 1，来源可追溯率 1.0。证据 `H:\tmp\offeru\profile-evolution-stress-20260911.json`、`H:\tmp\offeru\full-regression-reject-gate-20260911\pytest.log`。
...
- H 盘隔离 managed Chromium 无头业务路径已通过：从 UI 创建 Profile、保存岗位、完成 Replay Role Intelligence、Focus Plan、Interviewer Mode 自适应追问、8 轮回答、原文证据 Debrief、Learning Candidate 接受与 Profile 回写；console/page/API 错误均为 0。证据 `H:\tmp\offeru\browser-business-20260910-result.log`，数据库为 `H:\tmp\offeru\browser-business-20260910-data\djm.db`；这是合成 Replay，真实 Resume/Gmail 仍是外部门。
...
- 全站 adaptive route matrix 已通过：11 个主要路由 × 320/390/768/1440 四种视口，共 44 个页面实例，横向溢出、page error、console error、未预期 API 响应均为 0；动态 viewport 基础代码下重跑仍为 44/44，报告 `H:\\tmp\\offeru\\adaptive-route-matrix-20260910.json`，截图目录 `H:\\tmp\\offeru\\adaptive-route-matrix-20260910-artifacts`，重跑日志 `H:\\tmp\\offeru\\adaptive-route-matrix-foundation-rerun-20260910-result.log`。
...
- 动态 viewport foundation 已落地：根节点/App shell/Sidebar/ContextRail/BackendReadyGate/Onboarding/Resume workspace 使用 `100dvh` 优先的共享 classes，静态扫描已无页面级 `h-screen`/`min-h-screen`/`w-screen`；Email 连续 8 尺寸 live resize 保持状态和高度，API-on-resize 为 `0`，证据 `H:\\tmp\\offeru\\adaptive-resize-20260910.json`。布局盘点、目标系统、响应式矩阵和 scroll ownership 在 `docs/design/adaptive-ui/`。
