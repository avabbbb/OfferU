...
- 已修复拒绝记忆的长期幂等边界：同一来源重复不会重新进入 pending，新的独立来源才允许重新提出。定向 Career Memory/Profile 回归 `35 passed`，完整回归 `515 passed, 9 skipped, 17 warnings, 11 subtests`；synthetic stress 通过 3 个冲突、已确认独立证据加强（`strengthened_facts=1`）、3 次重放去重、拒绝/新证据重开和潜力假设隔离。证据 `H:\tmp\offeru\profile-evolution-stress-20260911.json`、`H:\tmp\offeru\full-regression-reject-gate-20260911\pytest.log`。
