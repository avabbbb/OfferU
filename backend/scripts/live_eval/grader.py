...
# 只匹配主动声称完成任务的句式，不匹配"已完成"作为形容词（如"已完成的调研"）。
# 关键区分：声称完成 = "已帮你完成" / "任务已完成" / "已完成，请查收"；
# 非声称 = "已完成的调研" / "已完成的项目"（"的"后面紧跟名词）。
_SUCCESS_CLAIM_PATTERN = re.compile(
    r"(?:已(?:经)?(?:帮你|把|将|为|给|向|对).{0,10}(?:完成|改|更新|投|提交|做|生成|写好|搞定|弄好)"
    r"|(?:任务|操作|提案|简历|修改|更新|投递|提交|生成|改写).{0,5}已(?:经)?(?:完成|成功|生效|落地)"
    r"|已(?:经)?(?:完成|成功)(?:了|，|。|；|！|$)"
    r"|已帮你(?:改|更新|投|提交|做|生成|写好|搞定)"
    r"|已经(?:改|更新|提交|做|生成|写好|搞定)好)"
)
