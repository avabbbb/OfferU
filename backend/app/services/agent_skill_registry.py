...
def _skill(
    ...
    *,
    ...
    return AgentSkill(id, name, group, status, description, mode, frozenset(tools), featured, order, missing, aliases)
...
_SKILLS = (
    ...
    _skill("auto_pipeline", "求职自动流水线", "pipeline", "partial", "从岗位证据开始编排筛选、调研和可审核简历提案；接受后才进入投递材料。", "job_workflow", ("get_profile", "list_jobs", "get_job", "list_resumes", "batch_triage", "run_scraper", "list_job_research_runs", "get_job_research", "start_job_research", "resume_job_research", "list_resume_optimizations", "get_resume_optimization", "prepare_resume_optimization", "review_resume_optimization"), featured=True, order=20, missing=("网页岗位存活检查",), aliases=("auto", "pipeline", "自动流水线")),
    ...
    _skill("scan_jobs", "岗位发现", "jobs", "partial", "先检查本地岗位库，必要时提议抓取新岗位。", "job_workflow", ("get_profile", "list_pools", "list_jobs", "job_stats", "run_scraper", "batch_triage"), featured=True, order=50, missing=("浏览器岗位存活检查",), aliases=("scan", "岗位扫描")),
    ...
    _skill("tailor_resume", "定制简历", "documents", "native", "先读取岗位调研证据，再从已验证档案事实生成逐项 diff；只有明确接受才创建正式简历。", "resume_workflow", ("get_profile", "list_jobs", "get_job", "list_resumes", "get_resume", "list_job_research_runs", "get_job_research", "start_job_research", "resume_job_research", "list_resume_optimizations", "get_resume_optimization", "prepare_resume_optimization", "review_resume_optimization"), featured=True, order=70, aliases=("resume", "简历", "定制简历")),
    ...
    _skill("follow_up", "跟进节奏", "applications", "native", "按确定性节奏计算到期跟进，生成草稿；只有确认已发送后才记账。", "skill_assistant", ("get_profile", "list_applications", "get_application_workspace", "list_application_events", "list_application_progress_candidates", "get_application_progress_candidate", "get_application_progress_overview", "list_follow_up_cadence", "list_calendar_events", "list_email_notifications", "email_connection_status", "list_email_accounts", "list_email_sync_runs", "get_email_sync_run", "list_career_artifacts", "get_career_artifact", "save_career_artifact", "record_follow_up", "review_application_progress", "update_application_status", "update_application_record", "sync_email_notifications", "revoke_email_account", "auto_fill_calendar"), featured=True, order=130, aliases=("followup", "follow_up", "跟进")),
    _skill("reply_watch", "回复识别", "applications", "native", "用 Gmail historyId 或 IMAP UID 增量同步邮箱，把消息保存为候选进展；只有使用者确认后才追加投递阶段事件。", "skill_assistant", ("list_applications", "get_application_workspace", "list_application_events", "list_application_progress_candidates", "get_application_progress_candidate", "get_application_progress_overview", "list_email_notifications", "email_connection_status", "list_email_accounts", "list_email_sync_runs", "get_email_sync_run", "list_career_artifacts", "get_career_artifact", "save_career_artifact", "review_application_progress", "sync_email_notifications", "revoke_email_account"), featured=False, order=140, aliases=("reply", "回复识别")),
    _skill("company_research", "公司与岗位调研", "research", "native", "以公开网页及使用者授权的本地只读浏览证据维护公司与岗位双档案，区分已引用、双来源印证、单一信号和未知，并只提炼匿名简历表达模式。", "skill_assistant", ("list_jobs", "get_job", "list_coding_agents", "list_job_research_runs", "get_job_research", "start_job_research", "resume_job_research", "list_authorized_research_sessions", "get_authorized_research_session", "start_authorized_research_session", "activate_authorized_research_read_only", "capture_authorized_research_page", "complete_authorized_research_session", "cancel_authorized_research_session"), featured=False, order=150, aliases=("deep", "research", "公司研究", "岗位调研")),
    ...
    _skill("profile_onboarding", "档案访谈", "profile", "native", "渐进发现职业证据缺口；学习信号先进入记忆收件箱，来源校验通过并确认后才写入。", "skill_assistant", ("get_profile", "list_profile_evidence", "list_learning_observations", "list_memory_inbox", "create_memory_proposal", "review_memory_proposal"), featured=True, order=170, aliases=("profile", "档案")),
    ...
    _skill("interview_plan", "面试冲刺计划", "interview", "native", "根据面试时间生成并保存有优先级的分时准备计划。", "skill_assistant", ("get_profile", "list_jobs", "get_job", "list_calendar_events", "list_interview_questions", "list_career_artifacts", "get_career_artifact", "save_career_artifact"), featured=False, order=195, aliases=("plan", "面试计划")),
    ...
    _skill("agent_inbox", "持续任务箱", "system", "native", "查看跨会话持久 Run、coding-agent 批处理和中断状态。", "skill_assistant", ("list_agent_runs", "list_batch_job_evaluations", "get_batch_job_evaluation", "resume_batch_job_evaluation"), featured=False, order=290, aliases=("inbox", "agent_inbox")),
)
