...
如果仓库根目录仍有 `OfferU.exe`，不要双击它。当前发现该文件是未纳入当前发布链的历史 `0.1.0` 二进制，内嵌旧的 `127.0.0.1:8000` / `127.0.0.1:3300` 入口，可能唤起无法连接的旧浏览器页面；本轮已将它可恢复地改名为 `OfferU-legacy-0.1.0.exe.disabled`。当前源码开发入口只有 `http://127.0.0.1:7410`，后端为 `http://127.0.0.1:8766`；`8080` 只用于可选 llama.cpp Provider，不是网页服务。该旧文件不由当前启动脚本使用，也不会被当前 Tauri Release artifact 收集。
