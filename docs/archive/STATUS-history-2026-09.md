...
- 2026-09-09 当前 checkout 复核：backend 全量回归 `518 passed, 9 skipped, 17 warnings, 11 subtests passed in 414.08s`；frontend typecheck 与 production build 均 exit `0`。日志和构建临时目录均位于 `H:\tmp\offeru`，未使用 C 盘测试目录。邮箱 Operation Registry 当前审计为 10 个只读操作、同步仅声明 `mailbox:read` 且要求确认，transport 契约另有 `9 passed`，未发现 send/reply/delete/archive/mark-read 等邮箱突变操作。证据：[full regression](H:/tmp/offeru/full-regression-resume-index-fix-20260910/pytest.log)、[frontend validation](H:/tmp/offeru/frontend-validation-current-20260911)、[email boundary](H:/tmp/offeru/audit-email-boundary-20260911.json)、[transport boundary](H:/tmp/offeru/email-read-only-transport-20260909.log)。
- 2026-09-10 Resume 真实 UI 复核补齐了此前发现的两个实际阻塞：`ProfileOnboarding` 的“原版机械解析”按钮现在挂接隐藏 PDF/DOCX 文件输入；机械解析同时识别常见英文章节标题（Education、Experience、Projects、Skills 等），不会把英文简历整份降级为 `custom:c_generic`。390×844 managed Chromium 无头路径已从建档、上传 H 盘 fixture、审核 4 条候选到确认生成档案完整通过，Profile 写入 8 个 section；`horizontal_overflow=false`、`console_errors=0`、`page_errors=0`、坏响应 `0`。定向 Resume review 为 `4 passed`，本轮完整 backend 回归为 `519 passed, 9 skipped, 17 warnings, 11 subtests passed in 429.24s`。证据：[browser Resume flow](H:/tmp/offeru/resume-browser-ui-20260912e/resume-import-browser-result.json)、[browser log](H:/tmp/offeru/resume-browser-ui-20260912e/resume-import-browser.log)、[targeted test](H:/tmp/offeru/profile-resume-english-headings-20260912/pytest.log)、[full regression](H:/tmp/offeru/full-regression-resume-english-20260912/pytest.log)。以上仍是 synthetic fixture；真实用户 Resume 与人工确认未完成。
- 2026-09-10 邮箱求职信号筛选补强：Gmail 30 天只读回补主题查询覆盖投递、招聘、录用、拒信、申请已提交、跟进及对应英文短语；正文/发件人仍经过同一相关性过滤，不扩大邮箱写权限。新增 GET-only 查询契约，邮箱切片 `10 passed`，完整 backend 回归 `520 passed, 9 skipped, 17 warnings, 11 subtests passed in 424.43s`，架构审计 `0 findings`。证据：[email query test](H:/tmp/offeru/email-query-boundary-20260910/pytest.log)、[full regression](H:/tmp/offeru/full-regression-email-query-20260910/pytest.log)、[architecture audit](H:/tmp/offeru/architecture-audit-email-query-20260910/audit.log)。真实 Gmail OAuth 仍未完成。
- 2026-09-10 同步状态投影现在也会对历史 `EmailAccount.last_error` 与 `EmailSyncRun.error` 做邮箱/电话/凭据脱敏，避免旧失败记录在“同步运行记录”中泄漏敏感文本；邮箱切片 `11 passed`，完整 backend 回归 `521 passed, 9 skipped, 17 warnings, 11 subtests passed in 429.93s`。证据：[status redaction test](H:/tmp/offeru/email-status-redaction-20260910/pytest.log)、[full regression](H:/tmp/offeru/full-regression-email-status-redaction-20260910/pytest.log)。真实 Gmail OAuth 仍未完成。
- 2026-09-10 Codex Bridge conformance 已升级为完整 Career context 读取门：真实 H 盘隔离任务自主调用 Profile、Resume、Job、learning observations、pending memory、Profile evolution、email accounts、email sync runs 共 10 个只读 Operation；`missing_context_reads=[]`、`mutations=0`、`grounding_verified=true`。相关切片 `65 passed`，完整 backend 回归 `521 passed, 9 skipped, 17 warnings, 11 subtests passed in 425.81s`，frontend typecheck/build 退出码均为 `0`，架构审计 `0 findings`。证据：[Codex context read](H:/tmp/offeru/codex-bridge-context-read-real-20260910.json)、[targeted](H:/tmp/offeru/bridge-context-read-targeted-20260910.log)、[full regression](H:/tmp/offeru/full-regression-bridge-context-20260910/pytest.log)、[commit 99da243](https://github.com/avabbbb/OfferU/commit/99da243)。该 Career context 使用合成隔离数据，真实 Resume/Gmail OAuth 仍未完成。
- 2026-09-10 Codex 故障收敛门补证：`backend/scripts/e2e/test_public_release_failure_recovery.py` 在 H 盘隔离 SQLite 中完成 auth blocked→retry、provider timeout failed→retry、backend restart recovery→retry，事件无重复；真实 Codex app-server 只终止本次创建的一个 PID，turn 在 `2.432s` 内 fail-closed，未假报 completed。为避免原生 app-server EOF 后等待协议超时，`CodexMainLoopAdapter` 现在传播有界 reader/process failure；定向测试 `1 passed`，完整 backend 回归 `522 passed, 9 skipped, 17 warnings, 11 subtests passed in 423.21s`，架构审计 `0 findings`。证据：[failure recovery](H:/tmp/offeru/failure-recovery-real-20260910/result.log)、[owned Codex kill](H:/tmp/offeru/codex-kill-real-20260910/result.json)、[adapter test](H:/tmp/offeru/codex-adapter-process-failure-20260910/pytest.log)、[full regression](H:/tmp/offeru/full-regression-codex-eof-20260910/pytest.log)、[commit 539a985](https://github.com/avabbbb/OfferU/commit/539a985)。所有 CWD、日志、数据库和测试临时文件仍位于 `H:\tmp\offeru`；真实 Resume/Gmail OAuth 与真实来源长期演化仍是外部门。
- 2026-09-10 Hosted Codex 认证门补强：隔离 `CODEX_HOME` 的真实 `account/read` 返回未登录时，`execute_deep_task` 在 `4.999s` 内持久化为 `failed` 并返回 `Codex authentication required`；HostedExecutor 关闭顺序改为优先等待 EOF，认证失败/进程终止不再留下 Proactor `ResourceWarning`。定向 runtime 回归 `19 passed, 2 subtests passed`，完整 backend 回归 `523 passed, 9 skipped, 17 warnings, 11 subtests passed in 426.89s`，架构审计 `0 findings`。真实 HostedExecutor PID 终止复跑在 `1.984s` 内为 durable `failed`，无 `session.completed`。证据：[auth unavailable](H:/tmp/offeru/codex-auth-unavailable-real-20260910c/probe.log)、[hosted process kill](H:/tmp/offeru/codex-hosted-kill-real-20260910b/probe.log)、[targeted](H:/tmp/offeru/codex-auth-gate-tests-20260910b/pytest.log)、[full regression](H:/tmp/offeru/full-regression-codex-auth-gate-20260910/pytest.log)、[commit dd41378](https://github.com/avabbbb/OfferU/commit/dd41378)。真实 Resume/Gmail OAuth 与真实来源长期演化仍未完成。
- 2026-09-10 Direct Codex Bridge 同步复核：`CodexMainLoopAdapter` 现在也在每次 Bridge turn 前检查 provider-owned `account/read`；登录态真实 conformance 仍自主发现并读取 10 个只读 Operation，`missing_context_reads=[]`、`mutations=0`、`grounding_verified=true`。定向 Bridge/runtime 回归 `49 passed, 2 subtests passed`，最新完整 backend 回归 `524 passed, 9 skipped, 17 warnings, 11 subtests passed in 428.32s`，架构审计 `0 findings`。证据：[Bridge conformance](H:/tmp/offeru/codex-bridge-context-read-auth-gate-20260910/conformance.log)、[targeted](H:/tmp/offeru/codex-direct-auth-tests-20260910/pytest.log)、[full regression](H:/tmp/offeru/full-regression-codex-bridge-auth-20260910/pytest.log)、[commit eea75df](https://github.com/avabbbb/OfferU/commit/eea75df)。真实 Resume/Gmail OAuth 与真实来源长期演化仍未完成。
- 2026-09-10 Direct Bridge 未认证 fail-closed 复核：隔离 `CODEX_HOME` 下 Bridge 在任何 Registry 调用前返回 `ok=false`、`failure_reason=Codex authentication required`、`tool_calls=0`、`mutations=0`；未产生 completed 结果。证据：[Bridge auth unavailable](H:/tmp/offeru/codex-bridge-auth-unavailable-20260910/result.json)。
- 2026-09-10 Codex app-server 握手顺序复核：Hosted 与 Direct Bridge 均遵循 `initialize → initialized → account/read → turn`；登录态真实 nonce/JSON/stream probe 与完整 Bridge Career context conformance 均通过。当前 HEAD 全量 backend `524 passed, 9 skipped, 17 warnings, 11 subtests passed in 430.51s`，定向 Bridge/runtime `49 passed, 2 subtests passed`，架构审计 `0 findings`。证据：[live](H:/tmp/offeru/codex-live-handshake-order-20260910/conformance.log)、[Bridge](H:/tmp/offeru/codex-bridge-context-read-auth-gate-20260910/conformance.log)、[full regression](H:/tmp/offeru/full-regression-codex-handshake-order-20260910/pytest.log)、[commit 44aca58](https://github.com/avabbbb/OfferU/commit/44aca58)。
- 2026-09-10 Agent 能力矩阵的认证状态分类已收紧：显式 `authentication required`/`login required` 失败现在投影为 `BLOCKED_AUTH`，不再混入普通 `ERROR`；H 盘隔离真实 Codex 未登录 live probe 返回 `BLOCKED_AUTH`，定向分类回归 `6 passed`，当前带默认 Profile 的 backend 全量回归 `525 passed, 9 skipped, 17 warnings, 11 subtests passed in 433.26s`。证据：[auth-required probe](H:/tmp/offeru/agent-conformance-auth-required-20260910/conformance.log)、[targeted](H:/tmp/offeru/agent-conformance-auth-classification-20260910/pytest.log)、[full regression](H:/tmp/offeru/full-regression-auth-classification-seeded-20260910/pytest.log)、[commit f381079](https://github.com/avabbbb/OfferU/commit/f381079)。真实 Resume/Gmail OAuth 与真实来源长期演化仍未完成。
- Resume 导入又修复了首条候选 `candidate_index=0` 的证据关联边界：此前 `or -1` 会把合法的 0 当成缺失，现已保留对应 `LearningObservation.id`；定向 Resume review 回归 `11 passed`，backend 全量回归同步升至 `518 passed`。H 盘 fixture 端到端解析/导入执行显示 8/8 候选均为 pending、8/8 均有 observation/proposal ID、正式 `ProfileSection` 写入为 `0`，事实门仍要求用户审核。证据：[targeted regression](H:/tmp/offeru/profile-resume-index-fix-20260910/pytest.log)、[fixture execution](H:/tmp/offeru/resume-parser-execution-20260910d/result.json)、[execution log](H:/tmp/offeru/resume-parser-execution-20260910d/run.log)；该数据仍是 synthetic fixture，不是用户 Resume。
...
- 记忆长期演化边界已修复：同一已拒绝 Proposal 由同一来源观察重复提交时返回终态 `duplicate`，只有新的独立 Observation 才能重新生成 pending Proposal。当前回归为 `515 passed, 9 skipped, 17 warnings, 11 subtests`；H 盘 synthetic stress 通过 3/3 冲突、已确认独立证据加强（`strengthened_facts=1`）、3 次重放去重、拒绝记忆、新证据重开与潜力假设隔离；演化报告类别计数为新增 7、加强 3、冲突 3、替代 3、拒绝 1、潜力假设 1，来源可追溯率 1.0。证据：[stress](H:/tmp/offeru/profile-evolution-stress-20260911.json)、[full regression](H:/tmp/offeru/full-regression-reject-gate-20260911/pytest.log)。
...
- Codex 通过 App Server → OfferU Bridge → Operation Registry 自行发现 doctor、manifest、playbook、operation list/schema；先后在空隔离库和非空隔离 Career context 中只读调用 5 个业务 Operation。最新非空验证读取 7 条 Profile 事实、1 份 Resume、1 个 Job 与 Evidence，`mutations=0`、`grounding_verified=true`。证据：[Codex bridge](H:/tmp/offeru/codex-bridge-seeded-real-20260910.json)、`H:\tmp\offeru\bridge-seeded-real-20260910-data\djm.db`。
...
- 全量 backend 回归为 `506 passed, 9 skipped, 17 warnings, 11 subtests passed in 263.47s`；frontend typecheck/build 通过，架构审计 `0 findings`。本轮 Resume 审核切片证据：[full regression](H:/tmp/offeru/full-regression-profile-review-final-20260910.log)、[slice evidence](H:/tmp/offeru/profile-t0-review-20260910.json)、[architecture audit](H:/tmp/offeru/architecture-audit-capability-projection-20260910.json)。
...
- 全站路由自适应矩阵已在 H 盘隔离环境通过：11 个主要路由（Today、Jobs、Resume、Applications、Interview、Calendar、Email、Profile、Settings、Studio、Optimize）覆盖 320×800、390×844、768×1024、1440×1000，共 44 个页面实例；`horizontal_overflow=0`、`page_errors=0`、console error case=0、未预期 API 响应=0。最新动态 viewport 基础代码下重跑仍为 44/44，截图与逐实例指标见 [adaptive route matrix](H:/tmp/offeru/adaptive-route-matrix-20260910.json)、`H:\tmp\offeru\adaptive-route-matrix-20260910-artifacts` 和 `H:\tmp\offeru\adaptive-route-matrix-foundation-rerun-20260910-result.log`；这是空状态/Replay UI 探针，不替代真实业务数据。
- 修复了 `frontend/src/app/studio/page.tsx` 的窄屏真实布局缺陷：`lg` 以下三栏改为模板→预览→设计控制单列，宽屏保留 `minmax` 三栏；所有列加 `min-w-0`，预览区使用 `dvh` 高度并保留紧凑视口最小可用高度。前端 typecheck/build 通过，Studio 320/390/768/1024/1440 五个视口与最新全站 44 实例矩阵均通过。证据：[Studio adaptive report](H:/tmp/offeru/adaptive-studio-20260910.json)、[matrix rerun](H:/tmp/offeru/adaptive-route-matrix-rerun-20260910-result.log)。
- 新增 Adaptive UI 基础层：`html/body/#root` 高度链补齐，App shell、Sidebar、ContextRail、BackendReadyGate、Onboarding 和 Resume workspace 统一使用 `100dvh` 优先的 viewport primitives；动态 resize 8 个连续尺寸通过，shell 高度始终匹配 viewport、Email 同意状态保持、resize 期间 API 请求为 `0`、console/page error 为 `0`。证据：[resize contract](H:/tmp/offeru/adaptive-resize-20260910.json)、`H:\tmp\offeru\adaptive-resize-20260910-artifacts`；布局盘点、目标架构和 scroll ownership 已落盘到 `docs/design/adaptive-ui/`。
