...
**数据库隔离门必须先于 pytest。** `app.database` 在模块导入时创建全局 engine，默认数据库是 `backend/djm.db`；当前多个测试会直接调用 `init_db()`、`async_session()` 和写 Operation。执行者必须在 Python 进程启动、任何 `app.*` 模块导入之前，把代码明确支持的 `DATABASE_URL` 指向一次性数据库，并记录用户数据库运行前后摘要。没有这组证据时，后端 pytest 状态是 `BLOCKED`，禁止直接运行下面的默认命令。
...
- 随机测试键、测试名称前缀和“只追加少量数据”都不算数据库隔离；测试结束后必须能丢弃整个一次性数据库。
- 不安装新依赖、不修改测试来追求绿色结果。
...
5. 用户数据库在运行前后摘要一致；测试产生的 Job、Profile、Memory、AuditLog 等只存在于一次性实例。
